...
def fsp_HyperparameterTuning_skopt(dataset_name):
    ...
    def objective(params):
        ...
        single_run = Parallel(n_jobs=-1)(
            delayed(FSP_cvLoss)(X_y, opt, predict_method=int(predict_method), cv=LeaveOneOut())
            for _ in range(10)
        )
    ...
    kmax = int( 10 + (90/9900)*(max(100,X_y.shape[0])-100) )
    ...
    if os.path.isfile(resultsAbsDirPath / f"{dataset_name}_fsp_evaluating_accuracy_KFold0.npz"):
        loaded_data = np.load(resultsAbsDirPath / f"{dataset_name}_fsp_evaluating_accuracy_KFold0.npz", allow_pickle=True)
        dataset_results = loaded_data['dataset_results'].tolist()
        x0 = []; y0 = []
        for R in dataset_results:
            opt = R['opt']
            # Add two initial points for each previous result (one for each predict_method)
            ...
            # Calculate and store the corresponding y values (1 - accuracy + 2*std)
    ...
    # Plot the convergence history to visualize optimization progress
    ...
    # Retrieve and print the best parameters from the optimizer
    ...
    if isinstance(dataset_name, str):
        ...
        #results_dir = "Results"
        if not os.path.exists(resultsAbsDirPath):
            os.makedirs(resultsAbsDirPath)

        # Define the result file name
        result_file = os.path.join(resultsAbsDirPath, f"{dataset_name}_fsp_HyperparameterTuning.csv")

        # Check if the file already exists
        file_exists = os.path.isfile(result_file)

        # Save opt, predict_method, and max_best_value in a CSV file
        with open(result_file, mode='a', newline='') as f:
            writer = csv.writer(f)

            # Write header only if the file doesn't already exist
            if not file_exists:
                writer.writerow(['optimizer', 'opt', 'predict_method', 'max_best_value'])

            # Write the results
            writer.writerow(["skopt", opt, predict_method, max_best_value])

    # Return results
    return opt, predict_method, max_best_value, OptimizeResult