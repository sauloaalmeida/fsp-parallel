...
class TestReadResult:
    ...
    def test_read_results1(self):
        projectRootAbsPath = Path(".").cwd()
        resultsAbsDirPath = projectRootAbsPath / "test" / "benchmark" / "fsp" / "Results" / "Results1"

        # Define the dataset results file name
        dataset_results_FullFileName = resultsAbsDirPath / f"DiabetesRisk_fsp_evaluating_accuracy_KFold0.npz"
        ...
        for result in results:
            ...
            #e os que a distancia diferentes de scipy, torch cpu e torch gpu
            if result['opt'].kmeans_method != 2 or result['opt'].distance_method not in [1, 3, 4]:
                continue
        ...
        ypoints = resultData[:,0]
        plt.plot(xpoints, ypoints, label="Scipy")

        ypoints = resultData[:,2]
        plt.plot(xpoints, ypoints, label="Torch CPU")

        ypoints = resultData[:,3]
        plt.plot(xpoints, ypoints, label="Torch GPU")
    ...
    def test_read_results2(self):
        projectRootAbsPath = Path(".").cwd()
        resultsAbsDirPath = projectRootAbsPath / "test" / "benchmark" / "fsp" / "Results" / "Results2"
