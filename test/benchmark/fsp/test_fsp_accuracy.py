...
def test_accuracy_fsp_one_base_one_option():
    result, outputFile = verify_accuracy_fsp(dataset_filenames=["Test1"], opt_labels=["opt1s0"])
...
def test_accuracy_fsp_one_base_two_options():
    result, outputFile = verify_accuracy_fsp(dataset_filenames=["Test1"], opt_labels=["opt1s0","opt12s1"])
    assert os.path.exists(outputFile)
    assert len(result) == 2
...
def test_accuracy_fsp_two_base_one_option():
    result, outputFile = verify_accuracy_fsp(dataset_filenames=["Test1","Test2"], opt_labels=["opt1s0"])
    assert os.path.exists(outputFile)
    assert len(result) == 2
    assert sum(1 for _ in open(outputFile)) == 3
    os.remove(outputFile)
...
def test_accuracy_fsp_two_base_two_optiona():
    result, outputFile = verify_accuracy_fsp(dataset_filenames=["Test1","Test2"], opt_labels=["opt1s0","opt7s0"])
    assert os.path.exists(outputFile)
    assert len(result) == 4
...
def verify_accuracy_fsp(dataset_filenames=None, opt_labels=None, k_fold=None, number_of_runs=10):
    """
    ...
    for dataset_filename in dataset_filenames:
        data = pd.read_csv(f"/home/saulo/workspace/projetos-python/fsp-python-gpu/test/benchmark/fsp/Datasets/{dataset_filename}.csv",header=None).values
        ...
        for opt_label in opt_labels:
            ...
            outputFileName=f"/home/saulo/workspace/projetos-python/fsp-python-gpu/test/benchmark/fsp/Results/FSP_{' '.join(dataset_filenames)}_KFold{k_fold}_NumberOfRuns{number_of_runs}.csv"
            pd.DataFrame(fsp_results).to_csv(outputFileName, index=False)

    return fsp_results, outputFileName
